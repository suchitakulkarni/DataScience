...
def plot_physics_comparison_results(x_anomalous, errors_pinn, errors_standard, threshold_pinn, threshold_standard, window_size, anomaly_idxs, pinn_weight, filename="results/physics_anomaly_score_comparison.pdf"):
    """
    ...
    ax1.set_ylabel('Amplitude (Scaled)', fontsize=14)
    ax1.legend(loc='upper right', fontsize=14)
    ax1.grid(True, linestyle=':', alpha=0.6)
    ax1.tick_params(axis='both', which='major', labelsize=12)
    
    # Subplot 2: LSTM Anomaly Score
    ax2 = axes[1]
    ...
    ax2.set_title(f'Anomaly Score: Physics-Informed LSTM (Weight={pinn_weight:.2f})', fontsize=14)
    ax2.set_ylabel('Error Magnitude (MSE)', fontsize=14)
    ...
    ax3.set_xlabel('Time Step Index', fontsize=14)
    ax3.set_ylabel('Error Magnitude (MSE)', fontsize=14)
...
def plot_reconstruction_comparison(x_anomalous, recon_pinn_windows, recon_standard_windows, window_size, anomaly_idxs, filename="results/reconstruction_comparison.pdf"):
    """
...
def plot_history(history, save_path="results/loss_curves.pdf"):
...
def get_detected_true_anomalies(anomaly_idxs, model_detections, window_size):
    ...
    Args:
        ...
        window_size (int): The size of the sliding window used by the LSTM.
...
def plot_detected_anomalies_comparison(x_anomalous, errors_pinn, errors_standard, 
                                       ...
                                       filename="results/detected_anomalies_comparison_TP.pdf"):
    """
    ...
    # Anomalies detected ONLY by LSTM (The one extra TP)
    ...
    # 2. Plot LSTM detections (Green Circles)
    ...
    # Anomalies detected ONLY by LSTM (The one that accounts for 28 vs 27)
