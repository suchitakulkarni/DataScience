...
from src.clean_uncertainty_discovery import BlindUncertaintyEstimator, comprehensive_blind_analysis, production_uncertainty_strategy

# Create results directory
os.makedirs('results', exist_ok=True)

def generate_synthetic_latency_data(n_samples=500, add_heteroscedasticity=True, add_nonlinearity=False, noise_level=2.0, seed=42):
    ...
    # Total latency
    measured_latency = physics_latency + nonlinear_component + measurement_noise + np.random.uniform(1, 3, n_samples)  # Base offset
...
def main():
    ...
    n_samples = st.sidebar.slider("Number of samples", 200, 1000, 500)
    add_hetero = st.sidebar.checkbox("Add Heteroscedasticity", True)
    add_nonlin = st.sidebar.checkbox("Add Non-linearity", False)
    noise_level = st.sidebar.slider("Noise Level", 0.5, 5.0, 2.0)
    ...
    if st.session_state.data_generated:
        ...
        train_data = generate_synthetic_latency_data(
            n_samples=n_samples,
            add_heteroscedasticity=add_hetero,
            add_nonlinearity=add_nonlin,
            noise_level=noise_level,
            seed=42
        )
        
        # Generate test data with some extrapolation
        test_data = generate_synthetic_latency_data(
            n_samples=200,
            add_heteroscedasticity=add_hetero,
            add_nonlinearity=add_nonlin,
            noise_level=noise_level,
        ...
        # Make some test points extrapolate beyond training range
        test_data['geo_distance_km'] = np.random.uniform(1500, 3000, 200)
        test_data['physics_latency_ms'] = test_data['geo_distance_km'] * 0.005
        if add_nonlin:
            nonlinear_comp = 0.000001 * test_data['geo_distance_km']**2
        else:
            nonlinear_comp = 0
        if add_hetero:
            noise_std = noise_level * (1 + test_data['geo_distance_km'] / 5000)
        else:
            noise_std = noise_level
        test_data['measured_latency_ms'] = (test_data['physics_latency_ms'] + nonlinear_comp +
                                          np.random.normal(0, noise_std, 200) +
        ...
        **Key Concepts:**
        ...
        with col2:
            ...
            st.write(f"**Linear R²:** {linearity_result['linear_r2']:.3f}")
            st.write(f"**Polynomial R²:** {linearity_result['poly_r2']:.3f}")
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["Statistical Tests", "Adaptive Physics", "Extrapolation-Aware", "Conformal Prediction"])
        ...
        with tab1:
            ...
            **Heteroscedasticity Results**:
            ...
            - Levene: $W = {hetero_result['levene_statistic']:.2f}$ (p = {hetero_result['levene_p_value']:.4f}) if hetero_result['levene_statistic'] is not None else "N/A"
